...
class ScaledDotProductAttention2(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        # 维度扩展
        q = q.unsqueeze(1)  # [5, 1, 1123, 1]
        k = k.unsqueeze(0)  # [1, 10, 231, 1]
        attn =  F.relu(torch.matmul(q / self.temperature, k.transpose(3, 4))-0.2)
        # 合并批次维度
        attn = attn.view(-1, attn.shape[2], attn.shape[3],attn.shape[4])  # [50, 1123, 231]
        if mask is not None:
            attn = attn.masked_fill(mask == 0, -1e9)
            # mask = mask.expand(-1, attn.size(1), -1, -1).to(attn.device)
            # attn[mask==0] = 0
        # self.label_same_matrix = self.label_same_matrix.to(attn.device)
        # attn = attn * self.label_same_matrix * 2 + attn * (1-self.label_same_matrix)
        ...
        # 步骤1：重组注意力权重维度
        attn = attn.view(q.shape[0], v.shape[0], -1, attn.size(-2), attn.size(-1))  # [5, 10, num_heads, user_len, item_len]

        # 步骤2：扩展商品特征维度
        v_expanded = v.unsqueeze(0).expand(q.shape[0], -1, -1, -1,-1)  # [5, 10, item_len, embed_dim]

        # 步骤3：批量矩阵乘法
        output = torch.matmul(
            attn,  # [num_heads, 5, 10, user_len, item_len]
            v_expanded  # [5, 10, embed_dim, item_len]
        ).sum(dim=1)  # 在用户维度聚合 [num_heads, 10, user_len, embed_dim]
...
class BGALayer(nn.Module):
    def __init__(self, n_head, channels, use_patch_attn=False, dropout=0.1):
    ...
    def forward(self, x, y, patch, patch2, attn_mask=None, need_attn=False):
        x = self.node_norm(x)
        y = self.node_norm(y)
        patch_x = x[patch]
        patch_y = y[patch2]
        patch_x_add, attn = self.node_transformer(patch_x, patch_y, patch_y, attn_mask)
        patch_x_add = self.node_ffn(patch_x_add)
        if need_attn:
